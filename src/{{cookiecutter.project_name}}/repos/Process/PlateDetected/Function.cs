...
namespace PlateDetected
{
  ...
  public class Function
  {
    ...
    public async Task<NumberPlateTrigger> FunctionHandler(NumberPlateTrigger payload, ILambdaContext context)
    {
      ...
      {
        throw new RandomProcessingError("Congratulations! A random processing error occurred!");
      }

      Table table = Table.LoadTable(dbClient, Environment.GetEnvironmentVariable("DDBTableName"));
      Document document = await table.GetItemAsync(payload.numberPlate.numberPlateString);
      if (document != null)
      {
        ...
        {
          throw new InsufficientCreditError("Driver for number plate " + payload.numberPlate.numberPlateString + "(" + document["ownerFirstName"] + ")" + document["ownerLastName"] + ") has insufficient credit (" + document["credit"] + ") for a charge of " + payload.charge);
        }
